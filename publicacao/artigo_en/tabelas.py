#!/usr/bin/env python3
"""tabelas.py — the tables of the English version, recomputed from the judgment files.

Mesma fonte de dados da versão em português: o inglês muda os rótulos, não os números.
...
# A vírgula decimal é da norma portuguesa; o artigo em inglês usa ponto, e os nomes dos
# modelos vão junto: "Tucano 3,7B" vira "Tucano 3.7B".
NOME = {k: v.replace(",", ".") for k, v in NOME.items()}
pt = lambda x, c=1, suf="": "—" if x is None else f"{x:.{c}f}{suf}"
ptd = lambda x, c=1, suf="": "—" if x is None else f"{x:+.{c}f}{suf}"

SAI = Path(__file__).resolve().parent
REG = [("nenhum", "no pre-training"), ("bruto", "raw text"), ("expandido", "cards")]
...
TAREFAS = [("discursiva", "on discursive questions"),
           ("peca_formal", "on legal document writing, formal criteria"),
           ("peca_material", "on legal document writing, merit criteria")]
...
def tab(n, cap, cabs, linhas, pe=""):
    th = "".join("<th" + (' class="n"' if c[0] == "#" else "") + ">" + c.lstrip("#") + "</th>"
                 for c in cabs)
    return (f'<figure class="tab" id="tab-{n}">\n<figcaption><b>Table {n}.</b> {cap}</figcaption>\n'
...
def fatorial(n: int, tarefa: str, rot: str, tam: dict[str, int]) -> str:
    ...
    cab = ["Model", "Pre-training", "#block 0"] + [f"#{ROTULO_BLOCO[b]}" for b in BLOCOS]
    L = []
    for m in MODELOS:
    ...
    return tab(n, f"Model performance {rot}.", cab, L)


ABREV = {"administrativo": "Admin.", "civil": "Civil", "constitucional": "Const.",
         "empresarial": "Corp.", "penal": "Crim.", "trabalhista": "Labour",
         "tributário": "Tax"}
...
def areas(n: int) -> str:
    ...
    """
    cab = ["Task", "Pre-training"] + [f"#{ABREV[a]}" for a in AREAS]
    L = []
    for tk, rot in TAREFAS:
        for i, (reg, rot_reg) in enumerate(REG):
            cs = [f'<td rowspan="3">{rot.replace("on ", "").replace("legal document writing, ", "document, ")}</td>'] if i == 0 else []
            cs.append(f"<td>{rot_reg}</td>")
            for a in AREAS:
            ...
            L.append(("<tr class='ctrl'>" if reg == "nenhum" else "<tr>") + "".join(cs) + "</tr>\n")
    return tab(n, "Performance by area of law on Tucano2 3.7B, averaged over the six blocks.",
               cab, L)
