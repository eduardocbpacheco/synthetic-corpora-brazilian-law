...
\end{abstract}
\begin{keyword}
adaptação de domínio \textperiodcentered{} direito brasileiro \textperiodcentered{} avaliação por modelo de linguagem \textperiodcentered{} LoRA
\end{keyword}
\end{frontmatter}
...
Os dois corpora de pré-treino, os seis blocos de ajuste fino, os julgamentos das 180 condições e as respostas geradas por elas estão depositados com identificador persistente e soma de verificação por arquivo, acompanhados de uma ficha de conjunto que descreve a coleta e os defeitos conhecidos.
...
\end{itemize}
O aninhamento foi verificado comparando os exemplos por conteúdo, e não pelo rótulo, porque um exemplo gerado herda o identificador do original que lhe serviu de semente. II $\subset$ III $\subset$ IV $\subset$ VI e I $\subset$ IV, mas \textbf{I e II são disjuntos}, sem um único exemplo em comum, assim como I e III.
...
\subsection{O bloco de carreiras ensina o esqueleto errado}
O bloco V contém 223 exemplos de concursos de carreira jurídica e nenhum material da OAB. Nas Tabelas 2 e 3 ele é a pior coluna de quase toda linha, e o déficit se concentra na parte formal. A hipótese é de gênero e faz uma previsão verificável: uma sentença não tem endereçamento, porque o juiz não se dirige a ninguém, e um parecer também não. A previsão se confirma com margem larga, pois o bloco V derruba os critérios de endereçamento entre 19,4 e 26,0 pontos percentuais nos quatro modelos, sem exceção, e a qualificação das partes entre 15,9 e 24,7.
...
\bibitem{r8} Pacheco, E., Russo, C. M. \emph{Juiz não é ranking: o que a tabela de classificação de corretores automáticos não consegue dizer}. Artigo companheiro, em preparação, 2026.
